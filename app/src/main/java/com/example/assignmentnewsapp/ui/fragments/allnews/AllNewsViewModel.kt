...
@HiltViewModel
class AllNewsViewModel @Inject constructor(private val apiHelper: ApiHelper) : BaseViewModel() {
  ...
  fun getListData(query:String, fromDate:String, toDate:String): Flow<PagingData<Article>> {
    return NewsRepo(apiHelper).getNewsStream(query,fromDate,toDate).cachedIn(viewModelScope)
  }
